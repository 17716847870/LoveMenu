...
export const foodRequestKeys = {
  ...
  list: (params?: Record<string, string | number | boolean>) => [...foodRequestKeys.lists(), params] as const,
...
export function useFoodRequests(status?: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<FoodRequest[]> => {
      ...
      const response = await http.get<FoodRequest[]>("/api/requests", { params });
...
export function useUpdateFoodRequest() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...data }: { id: string; status?: string; description?: string }) => {
      const response = await http.patch<FoodRequest>(`/api/requests/${id}`, data);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: foodRequestKeys.all });
