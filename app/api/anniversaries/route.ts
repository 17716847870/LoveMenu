...
export async function POST(req: Request) {
  try {
    ...
    const {
      ...
      repeatType, advanceDays = 0,
      emailTo, emailSubject, emailContent,
    } = body;

    if (!title || !calendarType || !month || !day || !repeatType || !emailTo || !emailSubject || !emailContent) {
    ...
    const record = await prisma.anniversary.create({
      data: {
        title, calendarType, month: Number(month), day: Number(day),
        weekday: weekday != null ? Number(weekday) : null,
        repeatType, advanceDays: Number(advanceDays),
        emailTo, emailSubject, emailContent,
        nextRemindAt,
      },
    });
