...
export default function MobileMenuListView({
  data,
  onDelete,
  onEdit,
  onPreviewImage,
}: MobileMenuListViewProps) {
  return (
    <div className="flex flex-col gap-3 mb-6 md:hidden">
      {data.map((item) => (
        <div
          key={item.id}
          className="bg-white rounded-2xl shadow-sm border border-pink-50/50 p-3 flex gap-3"
        >
          <div className="w-22 h-22 rounded-xl overflow-hidden shrink-0 bg-gray-50 relative border border-gray-50 group">
            {item.image ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-3xl bg-pink-50/30">
                🥘
              </div>
            )}
          ...
          <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
            <div>
              <div className="flex justify-between items-start">
                <h3 className="font-semibold text-gray-800 text-base truncate pr-2 leading-tight">
                  {item.name}
                </h3>
            ...
            <div className="flex justify-between items-end mt-2">
              <div className="flex flex-col gap-0.5">
                <span className="text-pink-600 font-bold text-sm leading-none">
                  💋 {item.kissPrice}
                </span>
