...
export default function MobileRequestListView({ data, onApprove, onReject, onDelete }: MobileRequestListViewProps) {
  return (
    <div className="flex flex-col gap-3 mb-6 md:hidden">
      {data.map((req) => {
        ...
        return (
          <div key={req.id} className="bg-white p-4 rounded-2xl shadow-sm border border-pink-50/50 flex gap-4">
            ...
            <div className="flex-1 flex flex-col justify-between py-0.5">
              <div>
                <div className="flex justify-between items-start">
                  <h3 className="font-bold text-gray-800 pr-2 line-clamp-1">{req.name}</h3>
                  <div className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap", statusConfig[req.status as keyof typeof statusConfig]?.color)}>
                    <StatusIcon className="w-3 h-3" />
                    {statusConfig[req.status as keyof typeof statusConfig]?.label}
                  </div>
                </div>
                <p className="text-xs text-gray-500 mt-1 line-clamp-2 leading-relaxed">{req.description}</p>
              </div>
