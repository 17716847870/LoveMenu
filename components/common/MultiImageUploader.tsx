...
export default function ImageUploader({
      ...
      if (file.size > maxSize * 1024 * 1024) {
        ...
        setImages(prev => prev.map(img => 
          img.id === newImages[i].id 
            ? { ...img, status: 'error' as const, error: `超过 ${maxSize}MB 限制` }
            : img
        ));
        continue;
      }
  ...
  return (
    <div className="space-y-3">
      {showTitle && (
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-gray-700">
            {isMultiple ? `图片（${images.filter(i => i.status === 'success').length}/${maxCount}）` : '图片'}
          </span>
      ...
      <div className={`grid gap-3 ${isMultiple ? 'grid-cols-3 md:grid-cols-4' : 'grid-cols-1'}`}>
      ...
      {previewIndex !== null && images[previewIndex] && (
        ...
        >
          ...
          <img
            src={images[previewIndex].remoteUrl || images[previewIndex].localUrl}
