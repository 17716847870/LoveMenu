...
> = {
  ...
  cute: {
    container:
      "bg-yellow-50 border-yellow-100 shadow-[4px_4px_0px_0px_rgba(250,204,21,0.2)]",
  ...
  night: {
    container:
      "bg-slate-900 border-slate-800 shadow-[0_0_15px_rgba(168,85,247,0.15)]",
    header: "text-purple-400",
    viewAll: "text-purple-500 hover:text-purple-300",
    rankItem:
      "bg-slate-800/50 border-purple-500/30 hover:border-purple-400/50 shadow-[0_0_10px_rgba(168,85,247,0.1)]",
    rankItemBorder: "border",
    rankBadge: (rank) =>
      "text-purple-400 font-bold border border-purple-500/50 rounded-lg px-2 bg-purple-500/10",
...
const RankItem = ({
  ...
  const getCountDisplay = () => {
    ...
    if (theme === "cute")
      return Array(Math.min(item.count, 5)).fill("⭐").join("");
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <span className={cn("font-medium", rank === 1 && "text-lg font-bold")}>
          {item.dishName}
        </span>
      </div>
...
export default function WeeklyFavoriteCard({
  list: propList,
}: WeeklyFavoriteCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn("flex flex-col", theme === "minimal" ? "gap-0" : "gap-3")}
      >
