...
import { Heart, Sparkles, List, Zap, ChevronRight, Plus } from "lucide-react";
...
> = {
  ...
  cute: {
    container:
      "bg-purple-50 border-purple-100 shadow-[4px_4px_0px_0px_rgba(168,85,247,0.2)]",
  ...
  night: {
    container:
      "bg-slate-900 border-slate-800 shadow-[0_0_15px_rgba(236,72,153,0.15)]",
...
export default function WishlistCard({ items: propItems }: WishlistCardProps) {
  ...
  const wishlistItems = useMemo(() => {
    ...
    return foodRequests.slice(0, 4).map((request) => ({
      id: request.id,
      name: request.name,
    }));
        ...
        <Link href="/wishlist">
          <motion.div
            ...
            className={cn(
              ...
              theme === "couple"
                ? "bg-pink-100/50 rounded-lg text-pink-600"
                : "",
              theme === "cute"
                ? "bg-purple-100/50 rounded-xl text-purple-600"
                : "",
              theme === "night"
                ? "bg-slate-800 rounded-lg text-pink-400 border border-pink-500/20"
                : ""
            )}
          >
