...
export default function EmergencyTip() {
  ...
  return (
    <motion.div
      ...
      className={cn("mx-4 mt-4 p-4 rounded-2xl flex items-center gap-3 text-sm font-bold shadow-sm", config.bg, config.text)}
    >
      <div className={cn("p-2 rounded-full bg-white/50", theme === 'night' && "bg-black/20")}>
        <Icon size={20} className={cn("animate-pulse")} />
      </div>
      <div className="flex flex-col">
        <span className="text-base">{tip}</span>
        <span className="text-xs opacity-70 font-normal mt-0.5">直接点击下方食物，最快速度下单</span>
      </div>
    </motion.div>
  );
}
